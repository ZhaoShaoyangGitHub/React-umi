...
class CalendarNormal extends Component<CalendarProps, CalendarState> {
    ...
    ToggleMonth = (type) => {
        ...
        switch (type) {
            case 'prev':
                ...
                this.getRangeTimeArr(timeRangeArr) // 天范围解析
                if (monthChange) {
                    monthChange(newYear_, newMonth_)
                }
                this.setState({
            ...
            case 'next':
                ...
                if (monthChange) {
                    monthChange(_newYear, _newMonth)
                }
                this.setState({
    ...
    render() {
        ...
        const { weekOpt, dayOpt, isShowCalendarTop, isShowNum } = this.props
        return (
                            ...
                            {isShowCalendarTop && (
                                <View className={styles.calendarTopView}>
                                    <View className={styles.leftBgView} onClick={this.ToggleMonth.bind(this, 'prev')}>
                                        ...
                                        <Image className={styles.leftView} src={attendanceImages.prevIcon} />
                                    </View>
                                    ...
                                    <View className={styles.rightBgView} onClick={this.ToggleMonth.bind(this, 'next')}>
                                        <Image className={styles.rightView} src={attendanceImages.nextIcon} />
                                    ...
                                    return (
                                        ...
                                        >
                                            <View className={styles.datesViewBox}>
                                                ...
                                                >
                                                    {isShowNum &&
                                                        this.getSchedulingNum(item.year, item.month, item.day) > 0 && (
                                                            <Text
                                                                className={styles.schedulingNum}
                                                                style={this.checkNum(item.year, item.month, item.day)}
                                                            >
                                                                {this.getSchedulingNum(item.year, item.month, item.day)}
                                                            </Text>
                                                        )}
                                                    <Text
                                                        decode
    ...
    checkNum = (year, month, day): object => {
        const { numOpt } = this.props
        if (numOpt) {
            for (let i = 0; i < numOpt.length; i++) {
                const _arr = numOpt[i].date.split('-')
                const _dotYear = _arr[0]
                const _dotMonth = _arr[1]
                const _dotDay = _arr[2]

                if (year == _dotYear && month == _dotMonth && day == _dotDay) {
                    return { backgroundColor: numOpt[i].color }
                }
            }
        }
        return {}
    ...
    getSchedulingNum = (year, month, day): number => {
        const { numOpt } = this.props
        if (numOpt) {
            for (let i = 0; i < numOpt.length; i++) {
                const _arr = numOpt[i].date.split('-')
