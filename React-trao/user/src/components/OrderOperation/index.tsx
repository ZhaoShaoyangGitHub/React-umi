...
class OrderOperation extends Component<OrderOperationProps, OrderOperationState> {
    ...
    handleAddComment = (trade) => {
        Taro.navigateTo({
            url: `/pages/AddComment/index?id=${trade.tradeId}&commentType=1`,
        })
    }
    ...
    render() {
        ...
        if (status === Status.Finished) {
            return (
                <View className={styles.buttonsWrapper}>
                    <View className={styles.primaryButton} onClick={() => handleDelete(trade)}>
                        删除订单
                    </View>
                    <View className={styles.primaryButton} onClick={() => this.handleAddComment(trade)}>
                        {evaluationStatus === 1 ? '待评价' : '已评价'}
                    </View>
                </View>
            )
        }
