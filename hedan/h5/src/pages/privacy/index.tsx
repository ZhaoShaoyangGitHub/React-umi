  ...
  render() {
    return (
      <div className={styles.wrapper}>
        ...
        盒DAN深知个人信息（含个人敏感信息）对你的重要性，并会全力保护你的个人信息安全。我们致力于维持你对我们的信任，恪守以下原则，保护你的个人信息：权责一致原则、目的明确原则、选择同意原则、最少够用原则、公开透明原则、确保安全原则、主体参与原则等。同时，盒DAN承诺，我们将按业界成熟的安全标准，采取相应的安全保护措施来保护你的个人信息。


        本隐私政策（以下简称“本政策”）适用于盒DAN的所有相关服务。在使用各项服务前，请你务必仔细阅读并透彻理解本政策，并做出你认为适当的选择。一旦你开始使用盒DAN的各项服务，即表示你已充分理解并同意本政策。需要提醒你注意，本隐私政策仅适用于我们所收集的信息，不适用于通过我们的服务而接入的第三方服务（包括任何第三方网站）收集的信息以及通过在我们服务中进行广告服务的其他公司或机构所收集的信息。
        ...
        1、盒DAN：指盒DAN软件或对盒DAN享有所有权或运营权的主体上海禾稻文化传播有限公司及其关联公司。
        ...
        3、个人敏感信息：指一旦泄露、非法提供或滥用可能危害人身和财产安全，极易导致个人名誉、身心健康受到损害或歧视性待遇等的个人信息。包括个人财产信息（银行账号、鉴别信息、交易和消费记录、流水记录等）、个人生物识别信息、个人身份信息（如身份证、军官证、护照、工作证）、网络身份标识信息（如系统账号、邮箱地址及前述有关的密码、口令、口令保护答案）、其他信息（个人电话号码、性取向、婚史、行踪轨迹、网页浏览记录、精确定位信息）等，本定义出自于GB/T35273《信息安全技术个人信息安全规范》。
        ...
        你在使用盒DAN提供的服务时，为了更好地实现你的目的，我们可能收集和使用你的个人信息。包括：

        1、你在注册盒DAN账号时提供的信息

        注册并登录盒DAN账号时，需要你提供手机号码或第三方账号（微信、微博 ）的信息，我们将通过发送短信验证码的方式来验证你的账号是否有效。你可根据自身需求补充填写个人信息（头像、昵称、简介、性别、城市、生日）。

        2、在你使用服务的过程中收集的信息

        使用本隐私政策“（二）我们如何使用你的个人信息”中所列举的盒DAN产品与/或服务时，我们可能会收集你使用的设备信息（包括操作系统、软件版本、语言设置、IP地址），浏览信息（包括搜索关键词、访问页面链接和访问时间），发表信息（包括文字、图片），位置信息，身份信息（包括认证设计师、品牌方工作人员时需要提供的身份证明、手机号码）。

        3、第三方向盒DAN提供的信息：

        你在第三方服务中使用盒DAN的时候，我们会收集第三方服务向我们提供的有关信息。
        ...
        （2） 个性化功能：你可以浏览、转发、收藏你感兴趣的资讯、动态，我们需要结合你的操作行为并通过算法分析，向你进行个性化推荐、展示或推送你可能感兴趣的特定信息。有的信息可能不是你喜欢的信息。

        （3） 搜索功能：你可以使用对关键字进行查询的服务，我们需要使用网络存储机制和应用数据缓存，收集你设备上的信息并进行本地存储，以提供更便捷的搜索服务。

        （4） 定位功能：你可以使用与地理位置相关的服务，在发布动态和编辑个人资料时自主选择是否上传定位信息。

        （5） 认证功能：你可以使用身份认证的服务。当你申请“认证设计师”时，我们需要收集你的姓名、验证身份信息。信息审核通过后，我们会为你的个人账号提供认证标识。

        （6）  营销活动：你可以参加由我们或第三方举办的线上或线下的营销活动，包括展览、市集等，我们会征得你的同意后，收集你的手机号码、用户id、真实姓名以提供更好的活动体验。

        （7） 客服功能：你可以向客服发起投诉、申诉或咨询，我们需要你提供真实姓名、手机号码、电子邮件。以及时地向你反馈投诉、申诉或咨询结果。

        （8） 经你同意的其他目的，包括但不限于向你发出产品和服务信息，或通过系统向你展示个性化的第三方推广信息，或在征得你同意的情况下与我们的合作伙伴共享信息以便他们向你发送有关其产品和服务的信息。有的信息有可能是你不希望接收的信息。
        ...
        尊重用户个人隐私是盒DAN的一项基本原则。除以下情形外，我们不会与任何公司、组织和个人分享你的个人信息：

        1、在获取明确同意的情况下共享：获得你的明确同意后，我们会与其他方共享你的个人信息。

        2、我们可能会按照司法机关或行政机关的要求，对外共享你的个人信息。

        3、与我们的关联公司共享：你的个人信息可能会与盒DAN的关联公司共享。我们只会共享必要的个人信息，且受本隐私政策中所声明目的的约束。关联公司如要改变个人信息的处理目的，将再次征求你的授权同意。
        ...
        (一)  我们已使用符合业界标准的安全防护措施保护你提供的个人信息， 防止数据遭到未经授权访问、公开披露、使用、修改、损坏或丢失。我们会采取一切合理可行的措施，保护你的个人信息。例如，在你的浏览器与 “服务”之间交换数据(如信用卡信息)时受SSL加密保护；我们同时对盒DAN网站提供 https 安全浏览方式；我们会使用加密技术确保数据的保密性；我们会使用受信赖的保护机制防止数据遭到恶意攻击；我们会部署访问控制机制，确保只有授权人员才可访问个人信息；以及我们会举办安全和隐私保护培训课程，加强员工对于保护个人信息重要性的认识。

        (二)  我们会采取一切合理可行的措施，确保未收集无关的个人信息。除非取得你的书面许可，我们将在且仅在你删除或注销账户后36个月保留你的个人信息， 超出此期限后，我们将会对你的个人信息进行匿名化处理或销毁。

        (三)  互联网并非绝对安全的环境，而且电子邮件、即时通讯、及与其他盒DAN用户的交流方式并未加密，我们强烈建议你不要通过此类方式发送个人信息。请使用复杂密码，协助我们保证你的账号安全。
        ...
        无论你何时使用盒DAN，我们都会尽一切可能保证你可以顺利访问自己的账户信息。如果这些信息有误，我们会努力提供各种方式来让你快速更新或删除账户内信息。为了使你拥有充分的能力保障你的隐私和安全，你拥有如下权利：
        ...
        如果你希望访问或编辑你的账户中的名号/昵称、头像、登录手机号、隐私设置等，可以通过在app中访问“我”，执行此类操作。

        搜索信息：

        你可以在搜索时，访问或使用清除功能，清除你的搜索历史记录。
        ...
        你可以通过删除信息、关闭移动设备功能、在盒DANpc端及移动端中进行隐私设置

        当你收回同意后，我们将不再处理相应的个人信息。但你收回同意的决定，不会影响此前基于你的授权而开展的个人信息处理。

        (五)  注销／删除账号

        你可以选择注销／删除盒DAN账号，账号被注销／删除后将不可恢复，你的个人信息将被删除或匿名化处理，建议你做好备份。注销／删除后手机号可自动解除绑定，之后可以与其它盒DAN账号绑定。

        如果你确定要注销账号，可以联系盒DAN客服提出申请。

        (六)  获取你的个人信息副本

        我们将根据你的书面请求，为你提供以下类型的个人信息副本：你的个人基本资料、账号信息。但请注意，我们为你提供的信息副本仅以我们收集的信息为限。
        ...
        对于你合理的请求，我们原则上不收取费用，但对多次重复、超出合理限度的请求，我们将视情收取一定成本费用。对于那些无端重复、需要过多技术手段(例如，需要开发新系统或从根本上改变现行惯例)、给他人合法权益带来风险或者非常不切实际(例如，涉及备份磁带上存放的信息)的请求，我们可能会予以拒绝。(例如，需要开发新系统或从根本上改变现行惯例)、给他人合法权益带来风险或者非常不切实际(例如，涉及备份磁带上存放的信息)的请求，我们可能会予以拒绝。
