import * as SaleOrderListApi from './apis'
import { BASEURL } from '../../config/index'

export default {
    namespace: 'SaleOrderList',
    state: {
        SaleOrderListDataList: [],
        list: [],
        // list: [
        //     {
        //         storeId: 1,
    ...
    effects: {
        *effectsDemo(action, { call, put }) {
            const { code, data } = yield call(SaleOrderListApi.demo, {})
            if (code === 'OK') {
                yield put({
                    type: 'save',
                    payload: data,
        ...
        *getList({ payload }, { call, put }) {
            console.log(2)
            const { code, data } = yield call(SaleOrderListApi.getOrderList, payload)
            if (code === 'OK') {
