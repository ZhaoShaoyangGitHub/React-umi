      ...
      <div className={styles.wrapper}>
        ...
        <div className={`${styles.wrapperBox} ${isMobile && styles.mobile}`}>
          ...
          {/* <div className={`${styles.explain} ${styles.cardBoxBStr}`}>
            *将于9月3日10点0分01秒在该页面开启奖池，23点59分59秒关闭奖池*
          </div> */}
          <div className={`${styles.cardBoxBStr} ${styles.cardBoxMargin} `}>
            获得抽奖权的小伙伴务必记得3日当天来此页面抽奖哦，过时作废哦●﹏●
          </div>
      ...
      return (
        <div className={styles.cardBox}>
          {Card({
            ...
            cont: (
              <>
                {`添加${detail.title}品牌下`}
                <span
                  dangerouslySetInnerHTML={{__html: detail.taskAddTxt}}
                ></span>
                至盒DAN玩具柜（玩具柜在“我”频道页面）
              </>
            ),
          })}
          ...
          {Card({
            ...
            cont: (
              <>
                {`首次发布${detail.title}的玩具`}
                <span
                  dangerouslySetInnerHTML={{__html: detail.taskReleaseTxt}}
                ></span>
                图片动态，带上
          ...
          {Card({
            ...
            cont: (
              <>
                <b>{`任意${progressNode().dynamic_2}天`}</b>
                {`发布带正确话题的${detail.title}玩具图片动态，`}
          ...
          {Card({
            ...
            cont: (
              <>
                <b>{`任意${progressNode().dynamic_3}天`}</b>
                {`发布带正确话题的${detail.title}玩具图片动态，`}
                <b>{`并分享 ≥ 其中任意${progressNode().share_3}条动态`}</b>
                <div className={styles.explain}>
                  {"*累计任务将9月2日23点59分59秒结束统计*"}
                </div>
              </>
            ),
          })}
