...
import { FixPhoneNumberProps, FixPhoneNumberState } from './index.interface'
import styles from './FixPhoneNumber.module.less'
import { Icons } from '../../assets/img/load'

@connect(({ FixPhoneNumber }) => ({
    ...FixPhoneNumber,
}))
class FixPhoneNumber extends Component<FixPhoneNumberProps, FixPhoneNumberState> {
    config: Config = {
    ...
    public time
    public time2
    constructor(props: FixPhoneNumberProps) {
        super(props)
        this.state = {
            verificationCode: '',
    ...
    handleSendCode = async () => {
        ...
        if (!isSended) {
            ...
            this.props.dispatch({
                type: 'UserLogin/sendCode',
                payload: { phone: personalInfo.phone },
                cb: (message) => {
                    Taro.showToast({
                        title: message || '发送成功',
                        icon: 'none',
    ...
    bindNewPhoneNumber = () => {
        ...
        this.props.dispatch({
            type: 'FixPhoneNumber/verifyPhone',
    ...
    bindNew = () => {
        ...
        this.props.dispatch({
            type: 'FixPhoneNumber/updateMobile',
            ...
            <View className={styles.FixPhoneNumberMain}>
                {isOld ? (
                    <View className={styles.fixOldPhone}>
                        <Image className={styles.fixPhoneImg} src={Icons.fixPhone} />
