 ...
 */
import * as JoinOrderListApi from './apis'
import { BASEURL } from '../../config/index'

export default {
    namespace: 'JoinOrderList',
    state: {
        JoinOrderListDataList: [],
        list: [],
        // list: [
        //     {
        //         storeId: 1,
    ...
    effects: {
        *effectsDemo(action, { call, put }) {
            const { code, data } = yield call(JoinOrderListApi.demo, {})
            if (code === 'OK') {
                yield put({
                    type: 'save',
                    payload: data,
        ...
        *deliverTrade(action, { call, put }) {
            const { code, data } = yield call(JoinOrderListApi.deliverTrade, action.params)
            if (code === 'OK') {
                action.cb(data)
            }
        },
        *getList({ payload }, { call, put }) {
            console.log(2)
            const { code, data } = yield call(JoinOrderListApi.getJoinOrderList, payload)
            if (code === 'OK') {
